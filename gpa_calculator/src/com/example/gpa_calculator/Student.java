...
public class Student {
	...
	private double CGA = 0.0;
	private double GGA = 0.0;
		...
		while (Answer0.equals("Y")) {
			...
			while (Answer1.equals("Y") && (year < MAX_STUDY_YEAR)) {
				...
				while (Answer2.equals("Y")
					...
					// System.out.print("year_sem_credit_counter[year][sem]=" +
					...
					System.out
							.print("Still have other course information to input for Year"
									+ (year + 1)
									+ " Sem"
									+ (sem + 1)
									+ "? Y/N: ");
			...
			System.out
					.print("Still have other course information to input ? Y/N");
	...
	// Users can type in their target CGA or target honor for graduation at the
	// first time of using this app
	/*
	 * while (true) { System.out.println("Please enter the target CGA form of
	 ...
	 * //to show the difference between target CGA and actual performance (1
	 * d.p.) public static void diff_targetCGA(String args[]) throws IOException
	 * {
	 * 
	 * 
	 * 
	...
	public void calTGA(int year, int sem) throws IOException {
		...
		// System.out.println("year_sem_course_counter[year][sem] = " +
		// year_sem_course_counter[year][sem]);
		// System.out.println("courseRecord[year][sem][i][0]) = " +
		// (courseRecord[year][sem][0][0]));
		// System.out.println("Integer.parseInt(courseRecord[year][sem][i][1]) = "
		// + Integer.parseInt(courseRecord[year][sem][0][1]));
		// System.out.println("GradeToNum(courseRecord[year][sem][i][2]) = " +
		// GradeToNum(courseRecord[year][sem][0][2]));
	...
	public void calCGA() throws IOException {
		...
		while (year < MAX_STUDY_YEAR) {
			...
			while (sem < MAX_SEMESTER) {
				...
				// System.out.println("sum_of_grade_point 3 = " +
				// sum_of_grade_point);
				...
				// System.out.println("year = " + year);
				// System.out.println("sem = " + sem);
				// System.out.println("sum_of_grade_point 4 = " +
				// sum_of_grade_point);
		 ...
		 * int sum_of_all_credit_taken = 0;
		 * 
		 * System.out.println("sum_of_all_credit_taken 1 = " +
		 * sum_of_all_credit_taken);
		 * System.out.println("year_sem_credit_counter[year][sem] 1 = " +
		 * year_sem_credit_counter[year][sem]);
		 * 
		 * 
		 * while (year < MAX_STUDY_YEAR) { while (sem < MAX_SEMESTER) {
		 * System.out.println("year_sem_credit_counter[year][sem] 2 = " +
		 * year_sem_credit_counter[year][sem]);
		 * 
		 * sum_of_all_credit_taken = sum_of_all_credit_taken +
		 * year_sem_credit_counter[year][sem]; sem++;
		 * 
		 * System.out.println("year = " + year); System.out.println("sem = " +
		 * sem); System.out.println("sum_of_all_credit_taken 2 = " +
		 * sum_of_all_credit_taken);
		 * System.out.println("sum_of_all_credit_taken 1 = " +
		 * sum_of_all_credit_taken); System.out.println("------------------"); }
		 * year++; }
		 */
		...
		while (year < MAX_STUDY_YEAR) {
			...
			while (sem < MAX_SEMESTER) {
				...
				if (year == 0) {
					...
					sum_of_all_credit_taken = sum_of_all_credit_taken
							+ (0.5 * year_sem_credit_counter[year][sem]);
		...
		while (year < MAX_STUDY_YEAR) {
			while (sem < MAX_SEMESTER) {
				...
				System.out.println("Year" + year + SemIntToWords(sem)
						...
						+ ", Credit" + courseRecord[year][sem][1] + ", Grade"
						+ courseRecord[year][sem][2]);
			}
		}
	}
	...
	public void printTGA() throws IOException {
		...
		while (Answer0.equals("Y")) {
			...
			while (Answer1.equals("Y")) {
				System.out
						.print("Which semster of year"
								+ (year + 1)
								+ " TGA you want to see? (Fall (Type 1) / Winter (Type 2) / Spring (Type 3) / Summer (Type 4) )");
				int sem = Integer.parseInt(input.nextLine()) - 1;

				System.out.println("The TGA of year" + (year + 1)
						+ SemIntToWords(sem) + " semester" + "  is "
						+ TGA[year][sem]);

				System.out.print("Still have other semster of year"
						+ (year + 1) + " TGA you want to see? Y/N: ");
				Answer1 = input.nextLine();

			}
			System.out
					.print("Still have other year TGA you want to see? Y/N: ");
			Answer0 = input.nextLine();
